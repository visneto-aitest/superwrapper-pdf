#!/usr/bin/env python3
"""
usage-kilo.py - Token usage report for Kilo CLI
Reads from ~/.local/share/kilo/kilo.db (SQLite).

Usage:
    usage-kilo.py --summary                    # Total summary
    usage-kilo.py --by-range                   # Breakdown by time period
    usage-kilo.py --sessions 20 --range week   # Sessions from last 7 days
    usage-kilo.py --projects --range month     # Projects from last 30 days
    usage-kilo.py --json                       # JSON output
"""
...
def cutoff_ms(days_ago=None):
    """Get cutoff timestamp in epoch ms. None = no cutoff (all)."""
    if days_ago is None:
        return 0
...
def parse_db(db_path, max_sessions=200, days_ago=None):
    """Parse Kilo SQLite database for session usage."""
        ...
        results.append(
            {
                ...
                "cost": total_cost,
                "duration_ms": time_updated - time_created,
                "messages": msg_count,
            }
        )
...
def show_summary(data, output_json=False):
    ...
    if output_json:
        ...
        print(f"  Kilo Usage Summary ({len(data)} sessions):")
        print("  " + "\u2500" * 62)
...
def show_by_range(output_json=False):
    """Show usage broken down by time periods."""
    ranges = [
        ("today", 0),
        ("week", 7),
        ("month", 30),
    ...
    for label, days in ranges:
        data = parse_db(KILO_DB, max_sessions=10000, days_ago=days)
        total_input = sum(s["input"] for s in data)
        total_output = sum(s["output"] for s in data)
        total_cr = sum(s["cache_read"] for s in data)
        total_cw = sum(s["cache_write"] for s in data)
    ...
    print("  Kilo Usage by Time Period:")
    ...
    for r in results:
        print("  {:<18s} {:>8d} {:>10s} {:>10s} {:>10s} {:>10s} ${:>9.4f}".format(
            ...
            fmt(r["input"]),
            fmt(r["output"]),
            fmt(r["cache_read"] + r["cache_write"]),
            fmt(r["reasoning"]),
            r["cost_usd"],
        ))
...
def show_sessions(data, output_json=False):
    if output_json:
        ...
        return
    if not data:
        print("  No Kilo sessions found.")
        return

    print(f"  Kilo Sessions (last {len(data)}):")
    print(
        "  {:<18s} {:<22s} {:<32s} {:<30s} {:>7s} {:>7s} {:>7s} {:>9s}".format(
            "Date", "Project", "Session", "Model", "Input", "Output", "Cache", "Cost"))
    print("  " + "\u2500" * 18 + " " + "\u2500" * 22 + " " + "\u2500" * 32 + " " + "\u2500" * 30 + " " + "\u2500" * 7 + " " + "\u2500" * 7 + " " + "\u2500" * 7 + " " + "\u2500" * 9)

    for s in data:
        date = fmt_date(s["date"])[:17]
        project = s["project"][:21]
        title = s["session"][:31]
        model = s["model"][:29]
        inp = fmt(s["input"])
        out = fmt(s["output"])
        cache = fmt(s["cache_read"] + s["cache_write"])
        cost = "${:.4f}".format(s["cost"] / 1_000_000)
        print(
            "  {:<18s} {:<22s} {:<32s} {:<30s} {:>7s} {:>7s} {:>7s} {:>9s}".format(
                date, project, title, model, inp, out, cache, cost))
...
def show_projects(data, output_json=False):
    ...
    sorted_p = sorted(
        projects.items(), key=lambda x: x[1]["input"] + x[1]["output"], reverse=True
    )
    print("  Kilo Usage by Project:")
    print("  {:<30s} {:>9s} {:>8s} {:>8s} {:>8s} {:>10s}".format(
        "Project", "Sessions", "Input", "Output", "Cache", "Cost"))
    print("  " + "\u2500" * 30 + " " + "\u2500" * 9 + " " + "\u2500" * 8 + " " + "\u2500" * 8 + " " + "\u2500" * 8 + " " + "\u2500" * 10)

    for name, p in sorted_p:
        total_cache = p["cache_read"] + p["cache_write"]
        cost = "${:.4f}".format(p["cost"] / 1_000_000)
        print("  {:<30s} {:>9d} {:>8s} {:>8s} {:>8s} {:>10s}".format(
            name[:29], p["sessions"], fmt(p["input"]), fmt(p["output"]),
            fmt(total_cache), cost))
...
def main():
    ...
    if action == "by-range":
        ...
        data = parse_db(KILO_DB, max_sessions if action == "sessions" else 200, days_ago=days_ago)
        if action == "summary":
