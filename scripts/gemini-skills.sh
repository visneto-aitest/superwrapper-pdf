...
# Gemini Agent Skills Manager - Combines skills inventory, backup, and restore functionality
# Usage: ./gemini-skills.sh [skills|list|backup|restore]

list_gemini_skills() {
    echo "=== Gemini Agent Skills Inventory ==="
    echo

    GLOBAL_PATHS=(
        "$HOME/.gemini/skills"
        "$HOME/.config/gemini/skills"
        "$HOME/.local/share/gemini/skills"
        "/usr/local/share/gemini/skills"
        "/opt/gemini/skills"
        "$HOME/.gemini/agent/skills"
        "$HOME/.config/gemini/agent/skills"
        "$HOME/.gemini/coding/skills"
        "$HOME/.config/gemini/coding/skills"
    )
    ...
    if command -v gemini &> /dev/null; then
        echo "Gemini Agent Info:"
        gemini skills list 2>/dev/null || gemini agent skills 2>/dev/null || echo "  Could not retrieve skills via gemini command"
        echo
    fi

    if [[ -f "$HOME/.gemini/config.json" ]]; then
        echo "Gemini Config: $HOME/.gemini/config.json"
        echo
    fi

    echo "=== End Gemini Agent Skills Inventory ==="
}

list_scripts() {
    echo "Listing Gemini scripts in scripts directory:"
    ls -la scripts/ | grep -E 'gemini' || echo "No scripts found"
}
...
case "$1" in
    ...
    restore)
        ...
        ;;
    *)
        echo "Usage: ./gemini-skills.sh [skills|list|backup|restore]"
        exit 1
        ;;
esac