...
# Pi Coding Agent Skills Manager - Combines skills inventory, backup, and restore functionality
# Usage: ./pi-skills.sh [skills|list|backup|restore]
...
list_scripts() {
    echo "Listing Pi scripts in scripts directory:"
    ls -la scripts/ | grep -E 'pi' || echo "No scripts found"
}
...
case "$1" in
    ...
    restore)
        ...
        ;;
    *)
        echo "Usage: ./pi-skills.sh [skills|list|backup|restore]"
        exit 1
        ;;
esac