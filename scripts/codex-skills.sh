...
# Codex Agent Skills Manager - Combines skills inventory, backup, and restore functionality
# Usage: ./codex-skills.sh [skills|list|backup|restore]

list_codex_skills() {
    echo "=== Codex Agent Skills Inventory ==="
    echo
    ...
    GLOBAL_PATHS=(
        ...
        "/opt/codex/skills"
        "$HOME/.codex/agent/skills"
        "$HOME/.config/codex/agent/skills"
        "$HOME/.codex/coding/skills"
        "$HOME/.config/codex/coding/skills"
    )
    ...
    if command -v codex &> /dev/null; then
        echo "Codex Agent Info:"
        codex skills list 2>/dev/null || codex agent skills 2>/dev/null || echo "  Could not retrieve skills via codex command"
        echo
    fi

    if [[ -f "$HOME/.codex/config.json" ]]; then
        echo "Codex Config: $HOME/.codex/config.json"
        echo
    fi

    echo "=== End Codex Agent Skills Inventory ==="
}

list_scripts() {
    echo "Listing Codex scripts in scripts directory:"
    ls -la scripts/ | grep -E 'codex' || echo "No scripts found"
}
...
case "$1" in
    ...
    restore)
        ...
        ;;
    *)
        echo "Usage: ./codex-skills.sh [skills|list|backup|restore]"
        exit 1
        ;;
esac